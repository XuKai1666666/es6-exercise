...
// 比传统的解决方案——回调函数和事件——更合理和更强大。它由社区最早提出和实现，
...
//   上面代码中，promise抛出一个错误，
//   就被catch()方法指定的回调函数捕获。注意，上面的写法与下面两种写法是等价的。
// 写法一
const promise = new Promise(function (resolve, reject) {
    try {
        throw new Error('test');
...
// 上面代码中，Promise 在resolve语句后面，再抛出错误，
// 不会被捕获，等于没有抛出。
...
// 123
// 上面代码中，someAsyncThing()函数产生的 Promise 对象，
...
//   上面代码运行完catch()方法指定的回调函数，
//   会接着运行后面那个then()方法指定的回调函数。
//   如果没有报错，则会跳过catch()方法。
...
// 上面的代码因为没有报错，跳过了catch()方法，
// 直接执行后面的then()方法。此时，要是then()方法里面报错，就与前面的catch()无关了。
...
//   上面代码中，catch()方法抛出一个错误，因为后面没有别的catch()方法了，
...
// 上面代码中，如果不使用finally方法，
// 同样的语句需要为成功和失败两种情况各写一次。有了finally方法，则只需要写一次。
...
// 自己定义了catch方法，那么它一旦被rejected，并不会触发Promise.all()的catch方法。
...
//   但是p2有自己的catch方法，该方法返回的是一个新的 Promise 实例，
