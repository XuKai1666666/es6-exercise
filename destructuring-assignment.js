...
// 赋值到对应的变量上，使用起来就会方便很多。例二将console.log赋值到log变量。
...
var { x = 3 } = { x: null };
x // null
// 属性x等于null，因为null与undefined不严格相等，
...
// 对于编译器来说，一个式子到底是模式，还是表达式，没有办法从一开始就知道，
// 必须解析到（或解析不到）等号才能知道。
...
({ p: (d) } = {}); // 正确
[(parseInt.prop)] = [3]; // 正确
// 上面三行语句都可以正确执行，因为首先它们都是赋值语句，
// 而不是声明语句；其次它们的圆括号都不属于模式的一部分。
