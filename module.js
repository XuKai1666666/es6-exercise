...
// 上面这些限制，模块都必须遵守。由于严格模式是 ES5 引入的，
// 不属于 ES6，所以请参阅相关 ES5 书籍，本书不再详细介绍了。
...
// 还是直接输出 1。1只是一个值，不是接口。正确的写法是下面这样。
...
// 如果处于块级作用域内，就会报错，
// 下一节的import命令也是如此。这是因为处于条件代码块之中，
// 就没法做静态优化了，违背了 ES6 模块的设计初衷。
...
a.foo = 'hello'; // 合法操作
// 上面代码中，a的属性可以成功改写，
// 并且其他模块也可以读到改写后的值。不过，这种写法很难查错，
// 建议凡是输入的变量，都当作完全只读，不要轻易改变它的属性。

// import后面的from指定模块文件的位置，可以是相对路径，
// 也可以是绝对路径。如果不带有路径，
// 只是一个模块名，那么必须有配置文件，告诉 JavaScript 引擎该模块的位置。
...
// 但是它们对应的是同一个my_module模块。也就是说，import语句是 Singleton 模式。

// 目前阶段，通过 Babel 转码，CommonJS 模块的require命令和
//  ES6 模块的import命令，可以写在同一个模块里面，
//  但是最好不要这样做。因为import在静态解析阶段执行，
